...
include("parameters_area.jl") #Load the parameters for the scan
...
area_iter = zeros(length(m0s),max_iter)
m_ncdms_iter = zeros(length(m0s),max_iter)
m_ncdms_iter[:,1] = m_ncdms
...
Threads.@threads for i=1
    for iter=1:max_iter
        ...
        if iter > 1
            if abs(area_iter[i,iter-1]-area_crit) > darea
                run_iteration = true
            else
                run_iteration = false
            end
        ...
        if run_iteration
            ...
            q = qs[i]
            k = ks[i]
            m_ncdm = m_ncdms_iter[i,iter]
            burst = bursts[i]
            use_fit = use_fits[i]

            println("m=",m0,"g, q=",q,", k=",k,", burst=",burst,", m_dm=",m_ncdm," GeV, iteration ",iter)
            ...
            writedlm(string("Input/psd_",i,"_sc.dat"),[e_class/T0 (T0^3/(mp*g_to_GeV)^2)*Φ1_class/1.0./ (e_class/T0).^2])
            ...
            area_iter[i,iter] = 1 - trapz(k_ev,pbh_int ./ cdm_int)/trapz(k_ev,cdm_int ./ cdm_int)

            if iter == 1
                if area_iter[i,iter] < area_crit
                    m_ncdms_iter[i,iter+1] = m_ncdms_iter[i,iter]/1.5
                else
                    m_ncdms_iter[i,iter+1] = m_ncdms_iter[i,iter]*1.5
                end
            elseif iter < max_iter
                m_ncdms_iter[i,iter+1] = m_ncdms_iter[i,iter-1]+(m_ncdms_iter[i,iter]-m_ncdms_iter[i,iter-1])/(area_iter[i,iter]-area_iter[i,iter-1])*(area_crit-area_iter[i,iter-1])
            end
        else
            area_iter[i,iter] = area_iter[i,iter-1]
            if iter < max_iter
                m_ncdms_iter[i,iter+1] = m_ncdms_iter[i,iter]
            end
        end
    end
end

writedlm("Output/area_crit.csv",[m0s ks qs bursts m_ncdms_iter[:,end] use_fits area_iter[:,end]])
